...
def resolve_handler(enabled: bool = False, callback: Optional[str] = None) -> Optional[EventHandler]:
    """
    Resolve the event handler explicitly by arguments, removing config dependencies.
    ...
    if callback:
        handler = EVENT_HANDLERS.get(callback)
        if handler:
            return handler
        
        logger.warning(
            f"Event handler '{callback}' not found in registry. "
