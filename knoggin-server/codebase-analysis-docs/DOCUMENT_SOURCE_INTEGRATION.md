...
### 1. `source_id` String Format is Fragile
Parsing meaning out of `source_id` via prefix conventions (`startswith("msg_")`) works initially, but doesn't scale cleanly. If a third source type is added (e.g., `note_`, `url_`), it means more `startswith` checks everywhere. Consider keeping `source_id` as a pure opaque ID and using the `source_type` column for branching logic instead of parsing the string.

### 2. `document_sources` Table May Duplicate FileRAG Metadata
The `FOLDER_UPLOAD_FILERAG_FEATURE.md` already designs a rich file metadata model (`project_documents` table) with `file_id`, `project_id`, `session_id`, `content_hash`, `relative_path`, etc. The `document_sources` table proposed here duplicates several of those fields. Consider making `document_sources` a **chunks-only table** that references the existing `project_documents.document_id` as a foreign key, rather than duplicating `user_name`, `session_id`, `project_id` on every chunk row.

### 3. Mixed Batches Need Prompt Awareness
The extraction prompts use `<speaker_context>` to tell the LLM about `[USER]` vs `[AGENT]` roles. If `[DOCUMENT]` chunks are mixed into the same batch, the LLM may try to apply speaker rules to document text (e.g., interpreting "I" and "my" as the user). **Recommendation**: Keep message batches and document batches separate initially, and use a prompt variant for documents that drops speaker context rules.

### 4. Relationship Extraction Prompt Needs a Document Variant
The relationship prompt (VEGAPUNK-02) has rules like "Never put {user_name} in entity_a" and "use user_connections for edges between {user_name} and another entity." For documents, there is no `{user_name}` in the text—it's third-party content. Either create a document-specific variant of that prompt or add a flag that disables `user_connections` for document batches.
