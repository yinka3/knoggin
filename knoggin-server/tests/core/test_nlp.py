"""Tests for src/core/nlp.py.

Uses real spaCy (en_core_web_md) and GLiNER models loaded once per session.
LLM interactions are mocked for orchestration testing.
"""
...
class TestExtractMentionsMocked:
    ...
    async def test_llm_returns_none(self, spacy_model, gliner_model):
        """Pipeline should still return PhraseMatcher + GLiNER results when LLM fails."""
        pipeline, _ = make_pipeline(spacy_model, gliner_model, llm_response=None)
