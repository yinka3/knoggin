...
from src.jobs.profile import ProfileRefinementJob
from src.jobs.merger import MergeDetectionJob
...
class TestProfileRefinementJob:
    ...
    async def test_no_dirty_entities(self):
        ...
        with patch("src.jobs.profile.emit", new_callable=AsyncMock):
...
class TestMergeDetectionJob:
    ...
    async def test_no_dirty_entities(self):
        ...
        with patch("src.jobs.merger.emit", new_callable=AsyncMock):
...
class TestTopicConfigJob:
    ...
    @pytest.mark.asyncio
    async def test_execute_no_conversation(self):
        ...
        with patch("src.jobs.topics.emit", new_callable=AsyncMock):
            result = await job.execute(ctx)
    ...
    @pytest.mark.asyncio
    async def test_execute_llm_returns_invalid_json(self):
        job, _ = self._make_topic_job_with_mock(llm_response="This is not JSON at all")
        redis = make_redis(
            zrevrange=AsyncMock(return_value=["turn_1"]),
            hmget=AsyncMock(return_value=[json.dumps({"role": "user", "content": "test"})]),
            set=AsyncMock(),
    ...
    @pytest.mark.asyncio
    async def test_execute_successful_evolution(self):
        new_config = {
            **TOPICS_RAW,
    ...
    @pytest.mark.asyncio
    async def test_execute_destructive_changes_rejected(self):
        old_with_more = {
            **TOPICS_RAW,
    ...
    @pytest.mark.asyncio
    async def test_execute_no_changes_needed(self):
    ...
    def test_sanitize_removed_topic_restored(self):
        old = TOPICS_RAW.copy()
    ...
    def test_sanitize_protected_topics_preserved(self):
        old = TOPICS_RAW.copy()
        new = {
            **TOPICS_RAW,
    ...
    def test_sanitize_hierarchy_preserved(self):
        old = {
            **TOPICS_RAW,
    ...
    def test_sanitize_new_topics_added(self):
        old = TOPICS_RAW.copy()
        new = {
            **TOPICS_RAW,
    ...
    def test_sanitize_new_topics_capped_at_three(self):
        old = TOPICS_RAW.copy()
        new = {
            **TOPICS_RAW,
    ...
    def test_sanitize_bulk_deactivation_rejected(self):
        old = {
            "General": {"active": True, "labels": [], "hierarchy": {}, "aliases": []},
            "Identity": {"active": True, "labels": ["person"], "hierarchy": {}, "aliases": []},
    ...
    def test_sanitize_label_cleanup(self):
        old = TOPICS_RAW.copy()
        new = {
            **TOPICS_RAW,
            "Fitness": {
    ...
    def test_sanitize_new_topic_missing_fields_defaulted(self):
        old = TOPICS_RAW.copy()
        new = {
            **TOPICS_RAW,
            "Fitness": {"labels": ["exercise"]},
        }
    ...
    def test_sanitize_single_deactivation_allowed(self):
        old = TOPICS_RAW.copy()
        new = {
            **TOPICS_RAW,
            "Education": {"active": False, "labels": ["university"], "hierarchy": {}, "aliases": []},
