...
from src.jobs.base import BaseJob, JobContext, JobResult
...
class TestScheduler:
    ...
    @pytest.mark.asyncio
    async def test_idle_seconds_no_activity_key(self):
        redis = MagicMock()
        redis.get = AsyncMock(return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_idle_seconds_recent_activity(self):
        redis = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_idle_seconds_old_activity(self):
        redis = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_record_activity_writes_timestamp(self):
        redis = MagicMock()
        redis.set = AsyncMock()
        scheduler = make_scheduler(redis)
    ...
    @pytest.mark.asyncio
    async def test_build_context_populates_all_fields(self):
        redis = MagicMock()
        redis.get = AsyncMock(return_value=None)
        scheduler = make_scheduler(redis)
    ...
    @pytest.mark.asyncio
    async def test_disabled_job_skipped_on_pending(self):
        redis = MagicMock()
        redis.set = AsyncMock()
        redis.delete = AsyncMock()
    ...
    async def test_successful_execution(self):
        ...
        with patch("src.jobs.scheduler.emit", new_callable=AsyncMock):
    ...
    async def test_execution_timeout(self):
        ...
        redis.get = AsyncMock(return_value=None)
        scheduler = make_scheduler(redis)
        scheduler.JOB_EXECUTION_TIMEOUT = 0.1

        job = StubJob(execute_delay=10)
        ctx = make_job_context(redis)
    ...
    def test_cleanup_removes_finished_task(self):
        scheduler = make_scheduler()
        mock_task = MagicMock()
    ...
    def test_cleanup_ignores_replaced_task(self):
        scheduler = make_scheduler()
        old_task = MagicMock()
        new_task = MagicMock()
