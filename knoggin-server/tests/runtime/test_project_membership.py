...
from knoggin_server.project.project_manager import ProjectManager, ProjectStatus
...
async def test_create_project_persists_metadata_and_default_topics(monkeypatch):
    ...
    monkeypatch.setattr(
        "knoggin_server.project.project_manager.uuid.uuid4",
        lambda: "project-1",
    )
...
async def test_delete_project_uses_cascading_postgres_boundary():
    ...
    assert "DELETE FROM public.projects" in write[1]
