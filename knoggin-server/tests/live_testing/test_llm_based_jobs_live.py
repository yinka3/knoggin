"""
Live LLM tests for VP-04/VP-05 parsers.

Validates that LLM output is parseable by deterministic parsers.
...
# Omar: job change, location change, buried details
VP04_STRESS_OMAR = """
...
# Jasmine: startup chaos, multiple pivots, cofounder stepping back
VP04_STRESS_JASMINE = """
...
# Derek: engagement, move, team change, injury, red herring (sister's boyfriend also named Derek)
VP04_STRESS_DEREK = """
...
# Chen family: pronoun resolution stress test
...
# Near-duplicates with contradicting facts
...
# Same name, temporal evolution (same person over time)
...
# Organizations with overlapping scope
...
# Multi-domain meandering conversation
...
class TestVP04Stress:
    """Complex multi-entity extraction scenarios."""
    ...
    async def test_omar_job_change_extracted(self, llm):
        ...
        for profile in result:
            all_facts.extend(profile.facts)
        raw_text = " ".join(all_facts).lower()
        print(f"[VP-04 Omar] Parsed facts: {all_facts[:10]}")
    ...
    async def test_jasmine_startup_chaos(self, llm):
        ...
        assert "a16z" in raw_text or "andreessen" in raw_text or "series a" in raw_text, \
    ...
    async def test_pronoun_resolution_chen_family(self, llm):
        """Chen family: track multiple people with pronouns."""
        enriched_facts = [
            {"content": f.content, "recorded_at": f.valid_at.isoformat(), "source_message": None}
...
class TestVP05Stress:
    """Ambiguous and tricky merge scenarios."""
...
class TestTopicEvolutionStress:
    ...
    async def test_scattered_topics_extracted(self, llm):
        """Rapid topic-switching should still extract coherent topics."""
        current_config = json.dumps(TOPICS_CURRENT, indent=2)
        user_content = (
            f"## Current Config\n{current_config}\n\n"
            f"## Recent Conversation\n{EVOLUTION_STRESS_SCATTERED}"
