...
class FakeJobEntities:
    ...
    def merge_into(self, primary_id, secondary_id):
        self.merge_calls.append((primary_id, secondary_id))
    ...
    async def get_hierarchy_candidates(
        ...
        return list(
            self.hierarchy_candidates.get(
                (topic, parent_type, tuple(child_types), min_weight),
                [],
            )
        )
...
async def test_detect_hierarchy_creates_project_scoped_edges_and_counts_successes():
    ...
    graph.hierarchy_candidates[("Work", "area", ("task",), 2)] = [
        {
            "parent_id": 101,
            "child_id": 202,
            "parent_name": "Knoggin",
