...
async def test_detect_merge_candidates_empty_cache_skips_graph_search(
    ...
    assert candidates == []
    assert graph.similar_entity_searches == []
...
async def test_collect_candidate_pairs_restricts_scan_to_dirty_ids(
    ...
    seed_entity(entities, graph, 303, "Knoggin", embedding=[0.0, 1.0])
    graph.similar_entities_by_id[101] = [(202, 0.95)]
    graph.similar_entities_by_id[303] = [(202, 0.95)]
...
async def test_collect_candidate_pairs_skips_self_and_dedupes_reverse_pairs(
    ...
    seed_entity(entities, graph, 101, "Robert Chen", embedding=[1.0, 0.0])
    seed_entity(entities, graph, 202, "Robert Chen Jr", embedding=[1.0, 0.1])
    graph.similar_entities_by_id[101] = [(101, 1.0), (202, 0.95)]
    graph.similar_entities_by_id[202] = [(101, 0.95)]
...
async def test_collect_candidate_pairs_rejects_weak_fuzzy_and_low_cosine_match(
    ...
    seed_entity(entities, graph, 202, "Linear", embedding=[0.0, 1.0])
    graph.similar_entities_by_id[101] = [(202, 0.95)]
...
async def test_collect_candidate_pairs_accepts_high_cosine_vector_similarity(
    ...
    entities, graph, _ = entity_manager_harness
    seed_entity(entities, graph, 101, "Alice", embedding=[1.0, 0.0])
    seed_entity(entities, graph, 202, "Linear", embedding=[0.95, 0.05])
    graph.similar_entities_by_id[101] = [(202, 0.95)]
...
async def test_collect_candidate_pairs_rejects_short_substring_noise(
    ...
    entities, graph, _ = entity_manager_harness
    seed_entity(entities, graph, 101, "AI", embedding=[1.0, 0.0])
    seed_entity(entities, graph, 202, "OpenAI", embedding=[0.0, 1.0])
    graph.similar_entities_by_id[101] = [(202, 0.95)]
...
async def test_collect_candidate_pairs_filters_generic_token_overlap(
    ...
    entities, graph, _ = entity_manager_harness
    seed_entity(entities, graph, 101, "project", embedding=[1.0, 0.0])
    seed_entity(entities, graph, 202, "project alpha", embedding=[0.0, 1.0])
    graph.similar_entities_by_id[101] = [(202, 0.95)]
...
async def test_collect_candidate_pairs_skips_missing_neighbor_profiles(
    ...
    entities, graph, _ = entity_manager_harness
    seed_entity(entities, graph, 101, "Robert Chen", embedding=[1.0, 0.0])
    graph.similar_entities_by_id[101] = [(999, 0.95)]

    pairs = await entities._collect_candidate_pairs([101], generic_tokens=set())

    assert pairs == {}
    assert graph.profile_lookups[-1] == {
        "entity_id": 999,
        "visible_project_ids": ["project-1"],
    }
...
async def test_detect_merge_candidates_returns_facts_scores_and_reasons(
    ...
    seed_entity(entities, graph, 202, "Robert Chen Jr", embedding=[1.0, 0.1])
    graph.similar_entities_by_id[101] = [(202, 0.95)]
    graph.facts_by_entity[101] = [fact_a]
    graph.facts_by_entity[202] = [fact_b]
