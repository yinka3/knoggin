...
from src.agent.community_tools import CommunityTools
...
class TestSaveMemory:
    ...
    async def test_saves_when_under_limit(self, ct, mock_base_tools):
        mock_base_tools.redis.hlen.return_value = 5
    ...
    async def test_blocked_at_10_memories(self, ct, mock_base_tools):
        mock_base_tools.redis.hlen.return_value = 10
    ...
    async def test_stores_correct_payload(self, ct, mock_base_tools):
        mock_base_tools.redis.hlen.return_value = 0
    ...
    async def test_exactly_9_memories_still_allows(self, ct, mock_base_tools):
        mock_base_tools.redis.hlen.return_value = 9
...
class TestSpawnSpecialist:
    ...
    @pytest.mark.asyncio
    @patch("src.agent.community_tools.get_config_value", return_value={"agent_model": "gemini-flash"})
    @patch("src.agent.community_tools.emit_community", new_callable=AsyncMock)
    ...
    async def test_spawn_limit_counts_only_spawned_prefix(self, ct):
        ...
        with patch("src.agent.community_tools.get_config_value", return_value={}), \
             patch("src.agent.community_tools.emit_community", new_callable=AsyncMock):
            result = await ct.spawn_specialist(
                name="Third Spawn",
                persona="Should work",
    ...
    @pytest.mark.asyncio
    @patch("src.agent.community_tools.get_config_value", return_value={})
    @patch("src.agent.community_tools.emit_community", new_callable=AsyncMock)
