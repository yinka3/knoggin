...
class LLMService:
    def __init__(
        ...
        if api_key:
            ...
            logger.info(
                f"LLMService initialized ({provider_label}) | extraction={extraction_model} | merge={merge_model} | agent={agent_model}"
            )
        else:
    ...
    def update_settings(
        ...
        if extraction_model:
            logger.info(
                f"LLMService: extraction model {self._extraction_model} -> {extraction_model}"
            )
            self._extraction_model = extraction_model
    ...
    async def _record_local_usage(
        ...
        try:
            ...
            logger.debug(
                f"Recorded approx usage ({model}): {total_tokens} tokens, ${total_cost:.6f} (approx)"
            )
        except Exception as e:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    logger.warning(
                        f"LLM call failed: {e}. Retrying ({attempt + 1}/{MAX_RETRIES})"
                    )
            ...
            try:
                ...
                if attempt < MAX_RETRIES - 1:
                    if content or tool_calls_detected:
                        ...
                        logger.error(
                            f"Stream failed ({model}) mid-generation: {e}. Cannot retry safely."
                        )
                        yield {
                            "type": "error",
                            "message": f"Stream interrupted mid-generation: {str(e)}",
                        }
                        return
                    logger.warning(
                        f"Stream failed ({model}): {e}. Retrying in {0.5 * (attempt + 1)}s..."
                    )
                    await asyncio.sleep(0.5 * (attempt + 1))
                    continue
                logger.error(
                    f"Stream failed ({model}) after {MAX_RETRIES} retries: {e}"
                )
