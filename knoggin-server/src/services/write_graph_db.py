...
async def write_batch_to_graph(
    ...
    for msg_result in extraction_result:
        msg_id = msg_result["message_id"] if isinstance(msg_result, dict) else msg_result.message_id
        pairs = msg_result["entity_pairs"] if isinstance(msg_result, dict) else msg_result.entity_pairs
