...
class CommunityStore:
    ...
    async def create_discussion(self, discussion_id: str, topic: str, agent_ids: List[str]):
        query = """
    ...
    async def delete_old_discussions(self, retention_days: int = 30) -> int:
        ...
        async def _delete(tx: AsyncManagedTransaction):
            result = await tx.run(query, cutoff=cutoff)
            record = await result.single()
            return record["deleted_discussions"] if record else 0
            
        async with self.driver.session() as session:
            count = await session.execute_write(_delete)
            if count > 0:
