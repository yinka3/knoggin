...
from db.readers.entity_reader import EntityReader
from db.readers.fact_reader import FactReader
from db.readers.graph_reader import GraphReader
from db.writers.entity_writer import EntityWriter
from db.writers.fact_writer import FactWriter
from db.writers.graph_writer import GraphWriter
...
class MemGraphStore:
    ...
    async def update_entity_profile(self, entity_id: int, canonical_name: str, embedding: List[float], last_msg_id: int):
        return await self._entity_writer.update_entity_profile(entity_id, canonical_name, embedding, last_msg_id)
    
    async def update_entity_canonical_name(self, entity_id: int, canonical_name: str) -> None:
        return await self._entity_writer.update_entity_canonical_name(entity_id, canonical_name)
    ...
    async def update_entity_checkpoint(self, entity_id: int, last_msg_id: int):
        return await self._entity_writer.update_entity_checkpoint(entity_id, last_msg_id)
    ...
    async def get_surrounding_messages(self, message_id: int, forward: int = 3, target_total: int = 10) -> List[Dict]:
        return await self._graph_reader.get_surrounding_messages(message_id, forward, target_total)

    async def get_facts_for_entity(self, entity_id: int, active_only: bool = True) -> List[FactRecord]:
    ...
    async def search_relevant_facts(self, entity_id: int, query_embedding: List[float], limit: int = 5) -> List[FactRecord]:
        return await self._fact_reader.search_relevant_facts(entity_id, query_embedding, limit)
    ...
    async def get_orphan_entities(self, protected_id: int = 1, orphan_cutoff_ms: int = 0, stale_junk_cutoff_ms: int = 0) -> List[int]:
        return await self._entity_reader.get_orphan_entities(protected_id, orphan_cutoff_ms, stale_junk_cutoff_ms)
    ...
    async def get_hierarchy_candidates(self, topic: str, parent_type: str, child_types: List[str], min_weight: int = 2) -> List[Dict]:
        return await self._graph_reader.get_hierarchy_candidates(topic, parent_type, child_types, min_weight)
    ...
    async def search_entities_by_embedding(self, embedding: List[float], limit: int = 10, score_threshold: float = 0.8) -> List[Tuple[int, float]]:
        return await self._entity_reader.search_entities_by_embedding(embedding, limit, score_threshold)
    ...
    async def list_entities(self, limit: int = 20, offset: int = 0, topic: str = None, entity_type: str = None, search: str = None) -> Tuple[List[Dict], int]:
        return await self._entity_reader.list_entities(limit, offset, topic, entity_type, search)
