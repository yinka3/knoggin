...
class AgentManager:
    ...
    async def create_agent(
        ...
        await self.resources.redis.hset(
            RedisKeys.agents(self.user_name), agent_id, json.dumps(config.to_dict())
        )
    ...
    async def set_default_agent(self, agent_id: str) -> bool:
        ...
        await self.resources.redis.set(
            RedisKeys.agents_default(self.user_name), agent_id
        )
    ...
    async def _seed_default_agents(self):
        ...
        await self.resources.redis.hset(
            agents_key, stella_id, json.dumps(stella.to_dict())
        )
