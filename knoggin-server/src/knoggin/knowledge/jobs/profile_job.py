...
class ProfileRefinementJob(BaseJob):
    ...
    async def execute(
        ...
        with logger.contextualize(
            ...
            logger.info(
                f"Profile refinement starting: {len(entity_ids)} entities to process{force_tag}{target_tag}"
            )
    ...
    async def _refine_user_profile(
        ...
        llm_input = [
            {
                ...
                "known_aliases": [
                    alias for alias in profile.get("aliases", [ctx.user_name])
                ],
            }
        ]
        ...
        final_active_facts, failed_invalidations = await self._apply_fact_changes(
            user_id, merge_result, existing_facts, valid_msg_ids, ctx.session_id
        )
        if failed_invalidations:
            dirty_key = RedisKeys.dirty_entities(ctx.user_name, ctx.session_id)
            await self.redis.sadd(dirty_key, str(user_id))
            logger.warning(
                f"Re-dirtied user entity {user_id}: {len(failed_invalidations)} invalidations failed"
            )
    ...
    async def _process_single_batch(
        ...
        async with self.batch_semaphore:
            ...
            for orig in batch:
                ...
                ) = await self._apply_fact_changes(
                    orig["ent_id"],
                    merge_result,
                    existing_facts,
                    valid_msg_ids,
                ...
                if failed_invalidations:
                    ...
                    logger.warning(
                        f"Re-dirtied entity {orig['ent_id']}: {len(failed_invalidations)} invalidations failed"
                    )

                embedding = await self._update_entity_embedding(
                    orig["ent_id"], orig["entity_name"], final_active_facts
                )
    ...
    async def _run_updates(
        ...
        tasks = [
            self._process_single_batch(
                ctx, batch, ents_to_facts, current_msg_id, valid_msg_ids
            )
    ...
    async def _apply_fact_changes(
        ...
        for fact_update in merge_result.new_contents:
            ...
            contradicted_ids = await self._detect_contradictions(
                content, embedding, active_existing, msg_id, session_id
            )
        ...
        if facts_to_create:
            try:
                count = await self.memgraph.create_facts_batch(
    ...
    async def _detect_contradictions(
        ...
        candidates: List[Tuple[FactRecord, float]] = sorted(
    ...
    async def _llm_judge_contradiction(
