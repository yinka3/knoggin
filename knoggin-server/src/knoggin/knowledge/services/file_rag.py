...
class FileRAGService:
    ...
    async def ingest_file(self, file_path: str, original_name: str) -> Dict:
        ...
        logger.info(
            f"Ingested file '{original_name}' -> {len(parent_chunks)} parents, {len(all_child_chunks)} children (session: {self.session_id})"
        )
        return file_meta
    ...
    def delete_file(self, file_id: str) -> bool:
        ...
        parent_keys_to_delete = [
            p_id
            for p_id in parent_store.keys()
            if p_id.startswith(f"{file_id}_parent_")
        ]
        ...
        logger.info(
            f"Deleted file '{file_meta['original_name']}' from session {self.session_id}"
        )
        return True
