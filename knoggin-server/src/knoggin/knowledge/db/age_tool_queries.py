...
class AgeToolQueries:
    ...
    async def get_hot_topic_context_with_messages(
        ...
        """
        query = self.client.build_cypher(cypher, "topic agtype, name agtype, aliases agtype, facts agtype, msg_ids agtype")
        
        try:
            data = await self.client.execute_read(query, (json.dumps({"hot_topics": hot_topic_names}),))
            
            topics_map = {}
            for row in data:
                t_name = row["topic"].strip('"') if isinstance(row["topic"], str) else row["topic"]
                if t_name not in topics_map:
                    topics_map[t_name] = {"entities": [], "message_ids": set(), "_entity_names": set()}
                
                e_name = row["name"].strip('"') if isinstance(row["name"], str) else row["name"]
                
                if e_name not in topics_map[t_name]["_entity_names"] and len(topics_map[t_name]["entities"]) < 3:
    ...
    async def search_entity(
        self, query: str, active_topics: Optional[List[str]] = None, limit: int = 5, connections_limit: int = 5, evidence_limit: int = 5
    ) -> List[Dict[str, Any]]:
        ...
        """
        try:
            id_res = await self.client.execute_read(search_sql, (f"%{clean_query}%", limit * 2))
            if not id_res:
                return []
            ...
            q = self.client.build_cypher(cypher, "id agtype, canonical_name agtype, aliases agtype, type agtype, topic agtype, last_mentioned agtype, last_updated agtype, facts agtype, conn_name agtype, conn_aliases agtype, conn_weight agtype, evidence_ids agtype, conn_context agtype, conn_facts agtype, parent_name agtype, children_count agtype")
            
            data = await self.client.execute_read(q, (json.dumps({
                "ids": entity_ids,
            ...
            for row in data:
                ...
                if eid not in entities:
                    entities[eid] = {
                        "id": eid,
                        "canonical_name": row["canonical_name"].strip('"') if isinstance(row["canonical_name"], str) else row["canonical_name"],
                        "aliases": row["aliases"] or [],
                        "type": row["type"].strip('"') if isinstance(row["type"], str) else row["type"],
                        "facts": row["facts"] or [],
                        "topic": row["topic"].strip('"') if isinstance(row["topic"], str) else row["topic"],
                        "last_mentioned": row["last_mentioned"],
                        "last_updated": row["last_updated"],
                        "top_connections": [],
                        "hierarchy": {
                            "parent": row["parent_name"].strip('"') if isinstance(row["parent_name"], str) else row["parent_name"],
                ...
                if row["conn_name"] and len(entities[eid]["top_connections"]) < connections_limit:
                    entities[eid]["top_connections"].append({
                        "canonical_name": row["conn_name"].strip('"') if isinstance(row["conn_name"], str) else row["conn_name"],
                        "aliases": row["conn_aliases"] or [],
                        "facts": row["conn_facts"] or [],
                        "weight": float(row["conn_weight"] or 1.0),
                        "context": row["conn_context"].strip('"') if isinstance(row["conn_context"], str) else row["conn_context"],
    ...
    async def get_related_entities(
        self, entity_names: List[str], active_topics: Optional[List[str]] = None, limit: int = 25
    ) -> List[Dict[str, Any]]:
        cypher = """
        ...
        LIMIT $limit
        """
        q = self.client.build_cypher(cypher, "source agtype, target agtype, target_facts agtype, connection_strength agtype, evidence_ids agtype, confidence agtype, last_seen agtype, context agtype")
        try:
            data = await self.client.execute_read(q, (json.dumps({
                "names": entity_names,
                "filter_topics": active_topics is not None,
            ...
            return [{
                "source": r["source"].strip('"') if isinstance(r["source"], str) else r["source"],
                "target": r["target"].strip('"') if isinstance(r["target"], str) else r["target"],
                ...
                "context": r["context"].strip('"') if isinstance(r["context"], str) else r["context"]
            } for r in data]
        except Exception as e:
    ...
    async def get_recent_activity(
        self, entity_name: str, active_topics: Optional[List[str]] = None, hours: int = 24
    ) -> List[Dict[str, Any]]:
        ...
        """
        q = self.client.build_cypher(cypher, "entity agtype, evidence_ids agtype, time agtype")
        try:
            data = await self.client.execute_read(q, (json.dumps({
                "name": entity_name,
                "cutoff": cutoff_ms,
            ...
            return [{
                "entity": r["entity"].strip('"') if isinstance(r["entity"], str) else r["entity"],
                "evidence_ids": r["evidence_ids"] or [],
                "time": r["time"]
            } for r in data]
        except Exception as e:
    ...
    async def _find_shortest_path(
        self, start_name: str, end_name: str, active_topics: Optional[List[str]] = None, max_depth: int = 4
        ...
        """
        q = self.client.build_cypher(cypher, "names agtype, node_topics agtype, evidence_ids agtype, has_inactive agtype")
        try:
            data = await self.client.execute_read(q, (json.dumps({
            ...
            if not data:
                return None
            row = data[0]
            return (row["names"], row["node_topics"], row["evidence_ids"], bool(row["has_inactive"]))
        except Exception as e:
    ...
    async def _find_active_only_path(
        ...
        RETURN [n IN path_nodes | n.canonical_name] AS names,
               ...
               [r IN path_rels | r.message_ids] AS evidence_ids
        """
        q = self.client.build_cypher(cypher, "names agtype, node_topics agtype, evidence_ids agtype")
        try:
            data = await self.client.execute_read(q, (json.dumps({
                "start_name": start_name,
                "end_name": end_name,
