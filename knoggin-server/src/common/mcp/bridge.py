...
def mcp_tools_to_schemas(tools: List[dict]) -> List[dict]:
    ...
    for tool in tools:
        ...
        if not name:
            logger.warning(
                f"[MCP Bridge] Skipping tool with no namespaced name: {tool}"
            )
            continue
        ...
        schemas.append(
            {
                ...
                "function": {
                    "name": name,
                    "description": f"[{server_label}] {description}"
                    if description
                    else f"[{server_label}] MCP tool",
    ...
    if schemas:
        logger.debug(
            f"[MCP Bridge] Converted {len(schemas)} MCP tools to function schemas"
        )
