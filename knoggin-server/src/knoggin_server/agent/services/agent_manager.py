...
class AgentManager:
    ...
    async def get_agent(self, agent_id: str) -> Optional[AgentConfig]:
        ...
        '''
        rows = await self.pg.execute_read(query, {"user_name": self.user_name, "agent_id": agent_id})
        if not rows:
            return None
    ...
    async def get_agent_by_name(self, name: str) -> Optional[AgentConfig]:
        ...
        query = '''
            ...
            LIMIT 1
        '''
        rows = await self.pg.execute_read(query, {"user_name": self.user_name, "name": name})
        if not rows:
            return None
    ...
    async def delete_agent(self, agent_id: str) -> bool:
        ...
        await self.pg.execute_write(query, {"user_name": self.user_name, "agent_id": agent_id})
    ...
    async def set_default_agent(self, agent_id: str) -> bool:
        ...
        await self.pg.execute_write(query_clear, {"user_name": self.user_name})

        query_set = "UPDATE public.agents SET is_default = true WHERE user_name = %(user_name)s AND agent_id = %(agent_id)s"
        await self.pg.execute_write(query_set, {"user_name": self.user_name, "agent_id": agent_id})
