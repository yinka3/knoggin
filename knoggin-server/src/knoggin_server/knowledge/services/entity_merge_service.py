...
class EntityMergeService:
    ...
    def __init__(self, postgres: PostgresClient, graph_client, redis=None):
    ...
    async def confirm(
        ...
        *,
        ...
        claimed = await self.postgres.execute_write(
            """
        ...
        merged = await self.graph_client.merge_entities(
            primary_id,
            duplicate_id,
    ...
    async def _snapshot(
        ...
        entities = await self.postgres.execute_read(
            """
            SELECT
                e.entity_id,
                e.user_name,
        ...
        facts = await self.postgres.execute_read(
            """
            SELECT *
            FROM facts
            WHERE project_id = %s
        ...
        relationships = await self.postgres.execute_read(
            """
            SELECT
                r.*,
                COALESCE(
        ...
        hierarchy = await self.postgres.execute_read(
            """
            SELECT *
