...
class DocumentService:
    ...
    @staticmethod
    def _matches_pattern(path: str, patterns) -> bool:
    ...
    @staticmethod
    def _ancestor_match(path: str, excluded_directories):
    ...
    async def get_scan_settings(self) -> FolderScanSettings:
        ...
        rows = await self._postgres.execute_read(
            """
            SELECT settings
    ...
    async def save_scan_settings(
        ...
        await self._postgres.execute_write(
            """
    ...
    def _build_folder_preview(
        ...
        normalized_overrides = {
            cls._normalize_relative_path(path, path)
        ...
        hidden_directories = {
            ...
            if not settings.include_hidden and cls._has_hidden_part(path)
        }
        default_directories = {
            path
            for path in directories
        ...
        custom_directories = {
            ...
            if (
                ...
                or cls._matches_pattern(path, settings.ignored_patterns)
            )
        }
        ...
        for path in sorted(normalized_entries):
            ...
            if len(content) > settings.max_document_size_bytes:
                ...
                continue
            if cls._matches_pattern(lower_name, SENSITIVE_FILE_PATTERNS):
                exclude(
                    path,
                    content,
            ...
            if (
                ...
                or (
                    extension != ".docx"
                    and cls._has_archive_signature(content)
                )
            ):
            ...
            if (
                ...
                or cls._has_executable_signature(content)
            ):
                exclude(
                    path,
                    content,
            ...
            hidden_directory = cls._ancestor_match(path, hidden_directories)
            ...
            default_directory = cls._ancestor_match(
            ...
            if default_directory is not None and not forced:
                ...
                continue
            if (
                cls._matches_pattern(path, DEFAULT_IGNORED_PATTERNS)
                and not forced
            ):
                exclude(
                    path,
            ...
            gitignored_directory = cls._ancestor_match(
            ...
            custom_directory = cls._ancestor_match(path, custom_directories)
            ...
            if (
                ...
                continue
            if (
                cls._matches_pattern(path, settings.ignored_patterns)
                and not forced
            ):
                exclude(
                    path,
    ...
    def _normalize_path_prefix(cls, path_prefix: Optional[str]) -> Optional[str]:
        if path_prefix is None:
            return None
        normalized = cls._normalize_relative_path(path_prefix, path_prefix)
    ...
    async def _get_visible_documents_by_reference(
        ...
        *,
        ...
        return await self._postgres.execute_read(
            """
            SELECT
                pd.document_id,
                pd.project_id,
    ...
    async def _list_folder_documents(
        ...
        *,
        ...
        """
        return await self._postgres.execute_read(query, tuple(params))
    ...
    async def search(
        ...
        *,
        ...
        normalized_relative_path = (
            self._normalize_relative_path(relative_path, relative_path)
        ...
        rows = await self._postgres.execute_read(sql, tuple(params))
        results = []
        for row in rows:
            result = dict(row)
