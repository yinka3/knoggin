...
from knoggin_server.agent.prompts import get_topic_evolution_prompt
...
class TopicConfigJob(BaseJob):
    ...
    async def execute(self, ctx: JobContext) -> JobResult:
        ...
        summary = (
            f"Topics updated — {'; '.join(summary_parts)}"
            if summary_parts
            else "Topics adjusted"
        )
    ...
    def sanitize_topic_evolution(old_config: dict, new_config: dict) -> dict | None:
        ...
        for topic_name, old_cfg in old_config.items():
            if topic_name not in new_config:
                logger.warning(
                    f"[TOPIC GUARD] LLM removed '{topic_name}', restoring as-is"
                )
        ...
        for topic_name in old_config:
            if topic_name in sanitized:
                sanitized[topic_name]["hierarchy"] = old_config[topic_name].get(
                    "hierarchy", {}
                )
        ...
        if len(new_topic_names) > 3:
            logger.warning(
                f"[TOPIC GUARD] Capping new topics from {len(new_topic_names)} to 3"
            )
