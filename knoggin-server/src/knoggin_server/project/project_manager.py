...
class ProjectManager:
    ...
    async def create_project(
        ...
        for allowed_id in allowed_projects:
            ...
            """
            await self.pg.execute_write(scope_query, {
                "user_name": self.user_name,
                "project_id": project_id,
                "readable": allowed_id
            })
    ...
    async def get_project(self, project_id: str) -> Optional[dict]:
        ...
        """
        rows = await self.pg.execute_read(query, {"user_name": self.user_name, "project_id": project_id})
        if not rows:
            return None
    ...
    async def get_readable_project_ids(self, project_id: str) -> List[str]:
        ...
        if allowed:
            ...
            """
            rows = await self.pg.execute_read(valid_query, {"user_name": self.user_name, "allowed": allowed})
    ...
    async def _validate_allowed_project_ids(
        ...
        """
        rows = await self.pg.execute_read(query, {"user_name": self.user_name, "requested": requested})
    ...
    async def update_project(
        ...
        if allowed_projects is not None:
            ...
            # Replace read scopes
            await self.pg.execute_write("DELETE FROM public.project_read_scopes WHERE user_name = %(user_name)s AND project_id = %(project_id)s", params)
            for allowed_id in validated_allowed:
                await self.pg.execute_write("""
                    INSERT INTO public.project_read_scopes (user_name, project_id, readable_project_id)
                    VALUES (%(user_name)s, %(project_id)s, %(readable)s)
        ...
        if updates:
            ...
            await self.pg.execute_write(f"UPDATE public.projects SET {set_clause} WHERE user_name = %(user_name)s AND project_id = %(project_id)s", params)
    ...
    async def archive_project(self, project_id: str) -> Optional[dict]:
        ...
        await self.pg.execute_write(query, {"status": ProjectStatus.ARCHIVED.value, "user_name": self.user_name, "project_id": project_id})
    ...
    async def reactivate_project(self, project_id: str) -> Optional[dict]:
        ...
        await self.pg.execute_write(query, {"status": ProjectStatus.ACTIVE.value, "user_name": self.user_name, "project_id": project_id})
    ...
    async def delete_project(self, project_id: str) -> Optional[dict]:
        ...
        await self.pg.execute_write(query, {"user_name": self.user_name, "project_id": project_id})
    ...
    async def _ensure_project_topics_config(
        ...
        rows = await self.pg.execute_read(
            """
            SELECT topic_config
