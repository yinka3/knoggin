...
async def get_stats(state: AppState = Depends(get_app_state)):
    ...
    try:
        graph_stats = await state.resources.memgraph.get_graph_stats()
    ...
    result = {
        ...
        "total_tokens": int(
            global_stats_raw.get("total_tokens", 0) if global_stats_raw else 0
        ),
        "total_cost": float(
            global_stats_raw.get("total_cost", 0.0) if global_stats_raw else 0.0
        ),
    }
...
async def get_stats_breakdown(state: AppState = Depends(get_app_state)):
    ...
    try:
        by_type, by_topic, top_connected = await asyncio.gather(
            state.resources.memgraph.get_entity_count_by_type(),
            state.resources.memgraph.get_entity_count_by_topic(),
            state.resources.memgraph.get_top_connected_entities(10),
        )
    except Exception as e:
        logger.error(f"Failed to get stats breakdown: {e}")
        by_type, by_topic, top_connected = [], [], []

    result = {"by_type": by_type, "by_topic": by_topic, "top_connected": top_connected}
