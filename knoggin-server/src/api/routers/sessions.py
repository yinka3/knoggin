...
async def create_session(
    ...
    if body and body.enabled_tools is not None:
        await state.session_manager.update_session_metadata(
...
async def export_all_sessions(state: AppState = Depends(get_app_state)):
    ...
    for session_id, metadata in sessions.items():
        try:
            if session_id in state.active_sessions:
                ...
                messages = await state.session_manager.get_session_history_readonly(
                    session_id
                )
...
async def get_session(session_id: str, state: AppState = Depends(get_app_state)):
    sessions = await state.session_manager.list_sessions()
...
async def export_session(session_id: str, state: AppState = Depends(get_app_state)):
