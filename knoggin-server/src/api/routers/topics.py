...
async def generate_topics_from_description(
    body: GenerateFromDescriptionRequest, state: AppState = Depends(get_app_state)
):
    ...
    if count >= 3:
        raise HTTPException(
            status_code=429,
            detail="Generation limit reached (3 attempts). Create the session or start a new configuration.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Topic generation failed due to an internal error."
        )
...
async def create_topic(
    ...
    return {"success": True, "topic": body.name}
