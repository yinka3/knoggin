...
async def approve_merge_proposal(
    ...
    if index < 1 or index > len(raw_proposals):
        raise HTTPException(
            status_code=400,
            detail=f"Invalid index. There are only {len(raw_proposals)} proposals.",
        )
    ...
    if (
        proposal["primary_id"] != body.primary_id
        or proposal["secondary_id"] != body.secondary_id
    ):
        raise HTTPException(
            status_code=409,
    ...
    if session_id in state.active_sessions:
        entities = state.active_sessions[session_id].entities
    ...
    success = await memgraph.merge_entities(primary_id, secondary_id)
    ...
    if entities:
        entities.merge_into(primary_id, secondary_id)

    try:
        all_facts = await memgraph.get_facts_for_entity(primary_id, False)
                ...
                if to_invalidate:
                    ...
                    for fact_id in to_invalidate:
                        try:
                            ...
                            pass
                    logger.info(
                        f"Invalidated {len(to_invalidate)} duplicate facts after manual merge"
                    )
...
async def undo_merge(
    ...
    if not entities:
        raise HTTPException(
            status_code=400, detail="Session must be active to undo a merge."
        )

    result = await _execute_undo(memgraph, entities, redis, snapshot, session_id)
...
async def _build_merge_snapshot(
    ...
    entity_data = await memgraph.get_entity_by_id(secondary_id)
    embedding = entities.get_embedding_for_id(secondary_id)

    facts_raw = await memgraph.get_facts_for_entity(secondary_id, False)
    facts = []
    if facts_raw:
        for f in facts_raw:
            facts.append(
    ...
    sec_rels = await memgraph.get_entity_relationships(secondary_id)
    pri_rels = await memgraph.get_entity_relationships(primary_id)
    parents = await memgraph.get_parent_entities(secondary_id)
    children = await memgraph.get_child_entities(secondary_id)
    ...
    return {
        ...
        "entity": {
            ...
            "type": (entity_data.get("type") or "unknown")
            if entity_data
            else "unknown",
            "topic": (entity_data.get("topic") or "General")
            if entity_data
            else "General",
        ...
        "parents": [
            {"id": p["id"], "name": p["canonical_name"]} for p in (parents or [])
        ],
        "children": [
            {"id": c["id"], "name": c["canonical_name"]} for c in (children or [])
        ],
    }
...
async def _execute_undo(
    ...
    await memgraph.write_batch([entity], [])
    ...
    if snapshot["facts"]:
        ...
        for fd in snapshot["facts"]:
            fact_objects.append(
                FactRecord(
                    ...
                    invalid_at=datetime.fromisoformat(fd["invalid_at"])
                    if fd.get("invalid_at")
                    else None,
        ...
        try:
            facts_restored = await memgraph.create_facts_batch(new_id, fact_objects)
        except Exception as e:
    ...
    for rel in snapshot["relationships"]:
        ...
        try:
            ...
            await memgraph.write_batch([], rel_data)
            rels_restored += 1
        except Exception as e:
            logger.warning(
                f"Failed to restore relationship to {rel['neighbor_name']}: {e}"
            )

    primary_pre_merge_neighbors = {
        r["neighbor_id"] for r in snapshot["primary_relationships"]
    }
    edges_removed = 0
    for rel in snapshot["relationships"]:
        neighbor_id = rel["neighbor_id"]
        ...
        if neighbor_id not in primary_pre_merge_neighbors:
            try:
                await memgraph.delete_relationship(primary_id, neighbor_id)
                edges_removed += 1
            except Exception as e:
                logger.warning(
                    f"Failed to remove transferred edge ({primary_id}, {neighbor_id}): {e}"
                )

    for parent in snapshot.get("parents", []):
        try:
            await memgraph.create_hierarchy_edge(parent["id"], new_id)
        except Exception as e:
            logger.warning(f"Failed to restore parent edge: {e}")

    for child in snapshot.get("children", []):
        try:
            await memgraph.create_hierarchy_edge(new_id, child["id"])
        except Exception as e:
