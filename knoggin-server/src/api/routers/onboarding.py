...
from knoggin.knowledge.services.topic_manager import (
    generate_topics as generate_topics_from_text,
)
from knoggin.session.onboarding import run_setup
...
ONBOARDING_QUESTIONS = {
    "guided": [
        ...
        {"id": 3, "question": "What are your main priorities or goals right now?"},
    ],
    "structured": [
        {
            "id": 1,
        ...
        {"id": 4, "question": "What are your main priorities or goals right now?"},
        {
            "id": 5,
...
async def generate_topics(
    ...
    text_block = "\n\n".join(
        f"Q: {r.question}\nA: {r.answer}" for r in body.responses if r.answer.strip()
    )
    ...
    try:
        ...
        merged = await asyncio.wait_for(
            generate_topics_from_text(
                state.resources.llm_service, text_block, user_name
            ),
            timeout=30.0,
        )
    except asyncio.TimeoutError:
        raise HTTPException(
            status_code=504, detail="Topic generation timed out. Please try again."
        )
...
async def save_topics(
    body: SaveTopicsRequest, state: AppState = Depends(get_app_state)
):
    if not body.topics:
...
async def get_questions(path: str):
    ...
    return {"path": path, "questions": ONBOARDING_QUESTIONS[path]}
