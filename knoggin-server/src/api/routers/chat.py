...
async def send_message(
    ...
    msg = Message(content=safe_message, timestamp=datetime.now(timezone.utc))
    ...
    async def event_stream():
        ...
        try:
            ...
            async for event in orchestrator.run_stream(
                ...
                else (agent.enabled_tools if agent else None),
                ...
                agent_rules=body.working_memory.get("rules")
                if body.working_memory
                else None,
                agent_preferences=body.working_memory.get("preferences")
                if body.working_memory
                else None,
                agent_icks=body.working_memory.get("icks")
                if body.working_memory
                else None,
            ...
            if final_response:
                ...
                if tool_calls_log:
                    ...
                    if first_tool_start is not None:
                        metadata["total_duration"] = round(
                            (time.time() - first_tool_start) * 1000
                        )
                if final_usage:
