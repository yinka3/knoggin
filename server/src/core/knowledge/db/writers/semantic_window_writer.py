...
class SemanticWindowWriter:
    ...
    async def enqueue_maintenance(
        ...
        *,
        ...
        """Durably queue post-completion maintenance without reopening a window."""

        user_name = require_scope_value(user_name, "user_name", "enqueue_maintenance")
        project_id = require_scope_value(project_id, "project_id", "enqueue_maintenance")
        window_id = self._uuid(window_id, "window_id")
        try:
            async with self.client.transaction() as cur:
                await cur.execute(
                    """
                    INSERT INTO public.project_semantic_window_maintenance (
                        window_id, project_id
                    )
                    SELECT window_id, project_id
                    FROM public.project_semantic_windows
                    WHERE window_id = %s
                      AND user_name = %s
                      AND project_id = %s
            ...
            self._raise_storage_write("enqueue_maintenance", exc)
    ...
    async def record_maintenance_failure(
        ...
        *,
        ...
        """Keep failed maintenance retryable while semantic completion remains final."""

        user_name = require_scope_value(
            user_name, "user_name", "record_maintenance_failure"
        )
        project_id = require_scope_value(
            project_id, "project_id", "record_maintenance_failure"
        )
        window_id = self._uuid(window_id, "window_id")
        error_summary = self._text(error_summary, "error_summary")[:2_000]
        try:
            async with self.client.transaction() as cur:
                await cur.execute(
                    """
                    UPDATE public.project_semantic_window_maintenance AS maintenance
                    SET status = 'pending',
                        attempt_count = attempt_count + 1,
                        last_error = %s,
                        completed_at = NULL,
                        updated_at = NOW()
                    WHERE maintenance.window_id = %s
                      AND maintenance.project_id = %s
                      AND EXISTS (
                          SELECT 1
                          FROM public.project_semantic_windows AS semantic_window
                          WHERE semantic_window.window_id = maintenance.window_id
                            AND semantic_window.project_id = maintenance.project_id
                            AND semantic_window.user_name = %s
                            AND semantic_window.stage = 'completed'
                      )
                    """,
                    (error_summary, window_id, project_id, user_name),
                )
                if cur.rowcount != 1:
                    raise ValueError("Completed semantic maintenance item is unavailable")
        except PsycopgError as exc:
            self._raise_storage_write("record_maintenance_failure", exc)
    ...
    async def complete_maintenance(
        ...
        *,
        ...
        """Mark one independently retried maintenance item complete."""

        user_name = require_scope_value(user_name, "user_name", "complete_maintenance")
        project_id = require_scope_value(project_id, "project_id", "complete_maintenance")
        window_id = self._uuid(window_id, "window_id")
        try:
            async with self.client.transaction() as cur:
                await cur.execute(
            ...
            self._raise_storage_write("complete_maintenance", exc)
