...
class EntityMention(StructuredLLMOutput):
    """One model-returned entity mention with a local message reference."""

    model_config = ConfigDict(extra="forbid")

    msg_id: str = Field(..., pattern=r"^m[1-9]\d*$")
    name: str = Field(..., description="Entity name as mentioned in the source text")
    type: str = Field(..., description="Semantic entity type")
