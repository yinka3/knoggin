...
async def test_preview_uses_configured_blocking_runner():
    calls = []
    ...
    await service.preview_folder(folder_name="repo", entries=[entry("src/main.py")])

    assert calls == ["build_folder_preview"]
...
@pytest.mark.storage
@pytest.mark.no_network
async def test_preview_rejects_a_relative_path_that_is_too_large(preview_service):
    with pytest.raises(ValueError, match="2048-byte limit"):
        await preview_service.preview_folder(
            folder_name="repo",
            entries=[entry("a" * 2049)],
        )
...
@pytest.mark.storage
@pytest.mark.no_network
async def test_preview_excludes_generated_minified_and_vendor_files(preview_service):
    preview = await preview_service.preview_folder(
        folder_name="repo",
        entries=[
            entry("vendor/library.py"),
            entry("src/schema.generated.py"),
            entry("src/banner.py", b"# Code generated by tool. DO NOT EDIT.\n"),
            entry("web/app.js", b"const x=0;" + b"x" * 20_000),
            entry("src/main.py"),
        ],
    )

    assert [item.relative_path for item in preview.included] == ["src/main.py"]
    assert {item.relative_path: item.reason for item in preview.excluded} == {
        "src/banner.py": "generated_file",
        "src/schema.generated.py": "generated_file",
        "vendor/library.py": "default_directory_ignore",
        "web/app.js": "minified_file",
    }
