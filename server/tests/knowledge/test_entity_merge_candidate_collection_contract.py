...
async def test_collect_candidate_pairs_accepts_high_cosine_vector_similarity(
    ...
    assert pairs == {
        (101, 202): {
            "fuzz_score": 0,
            "cosine_score": pytest.approx(0.9986178293325098),
...
async def test_detect_merge_candidates_returns_facts_scores_and_reasons(
    ...
    assert candidates[0]["fact_support"] == "neutral"
    assert candidates[0]["fact_support_pairs"]
...
@pytest.mark.storage
@pytest.mark.no_network
async def test_generic_tokens_cache_uses_alias_version(entity_manager_harness):
    entities, knowledge_store, _ = entity_manager_harness
    seed_entity(
        entities,
        knowledge_store,
    ...
    first_version = entities.get_alias_version()

    first = entities._build_generic_tokens(first_version)
    second = entities._build_generic_tokens(first_version)
    entities._build_generic_tokens.cache_clear()

    assert first is second
    assert entities._build_generic_tokens.cache.maxsize == 5


@pytest.mark.storage
@pytest.mark.no_network
async def test_generic_tokens_cache_recomputes_after_alias_version_bump(
    entity_manager_harness,
):
    entities, knowledge_store, _ = entity_manager_harness
    entities.generic_token_freq = 2
    seed_entity(entities, knowledge_store, 101, "project alpha", embedding=[1.0, 0.0])
    seed_entity(entities, knowledge_store, 202, "memory beta", embedding=[0.0, 1.0])
    first_version = entities.get_alias_version()
    first_tokens = entities._build_generic_tokens(first_version)

    entities.commit_new_aliases(202, ["project beta"])
    second_tokens = entities._build_generic_tokens(entities.get_alias_version())

    assert "project" not in first_tokens
    assert "project" in second_tokens


@pytest.mark.storage
@pytest.mark.no_network
async def test_vector_merge_candidate_with_nli_entailment_is_surfaced(
    entity_manager_harness,
):
    entities, knowledge_store, embedding = entity_manager_harness
    embedding.text_pair_labels = ["entailment"]
    fact_a = make_fact(101, "Alice leads the memory graph project.")
    fact_b = make_fact(202, "Alice leads the memory graph project.")
    seed_entity(
        entities,
        knowledge_store,
        101,
    ...
    assert len(candidates) == 1
    assert candidates[0]["reasons"] == ["vector_similarity"]
    assert candidates[0]["fact_support"] == "entailment"
    assert candidates[0]["fact_support_pairs"][0]["label"] == "entailment"


@pytest.mark.storage
@pytest.mark.no_network
async def test_vector_merge_candidate_with_nli_contradiction_is_rejected(
    entity_manager_harness,
):
    entities, knowledge_store, embedding = entity_manager_harness
    embedding.text_pair_labels = ["contradiction"]
    seed_entity(entities, knowledge_store, 101, "Alice", embedding=[1.0, 0.0])
    seed_entity(
        entities,
        knowledge_store,
    ...
    knowledge_store.similar_entities_by_id[101] = [(202, 0.95)]
    knowledge_store.facts_by_entity[101] = [
        make_fact(101, "Alice works at OpenAI.")
    ]
    knowledge_store.facts_by_entity[202] = [
        make_fact(202, "Design Alice works at Anthropic.")
    ]
...
@pytest.mark.storage
@pytest.mark.no_network
async def test_vector_merge_candidate_with_neutral_facts_is_rejected(
    entity_manager_harness,
):
    entities, knowledge_store, embedding = entity_manager_harness
    embedding.text_pair_labels = ["neutral"]
    seed_entity(entities, knowledge_store, 101, "Alice", embedding=[1.0, 0.0])
    seed_entity(
        entities,
        knowledge_store,
    ...
    knowledge_store.similar_entities_by_id[101] = [(202, 0.95)]
    knowledge_store.facts_by_entity[101] = [make_fact(101, "Alice likes graphs.")]
    knowledge_store.facts_by_entity[202] = [make_fact(202, "Design Alice likes UI.")]
...
@pytest.mark.storage
@pytest.mark.no_network
async def test_sparse_fact_vector_candidate_requires_very_high_cosine(
    entity_manager_harness,
):
    entities, knowledge_store, _ = entity_manager_harness
    seed_entity(entities, knowledge_store, 101, "Alice", embedding=[1.0, 0.0])
    ...
    knowledge_store.similar_entities_by_id[101] = [(202, 0.95), (303, 0.99)]

    candidates = await entities.detect_merge_entity_candidates(dirty_ids={101})

    assert [candidate["secondary_id"] for candidate in candidates] == [303]
    assert candidates[0]["fact_support"] == "insufficient_facts"


@pytest.mark.storage
@pytest.mark.no_network
async def test_vector_candidate_rejects_incompatible_type_or_topic(
    entity_manager_harness,
):
    entities, knowledge_store, _ = entity_manager_harness
    seed_entity(
