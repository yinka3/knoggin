...
@pytest.mark.requires_postgres
@pytest.mark.no_network
async def test_retention_writer_handles_a_large_expired_terminal_dataset(
    real_postgres_client,
):
    writer = RetentionWriter(real_postgres_client)
    user_name = "retention-bulk-test"
    project_id = "retention-bulk-project"
    old = get_now() - timedelta(days=200)
    ...
    await real_postgres_client.execute(
        ...
        SELECT 'bulk-suggestion-' || n, %s, %s, 'session-1', n,
               'mention-' || n, 'tool', 'General', n, 'Candidate ' || n,
               0.5, %s
        FROM generate_series(1, 250) AS series(n)
        """,
        (user_name, project_id, old),
    )
    await real_postgres_client.execute(
        """
        INSERT INTO agent_tool_audits (
        ...
        SELECT md5('bulk-audit-' || n)::uuid, %s, 'agent-1', %s, 'session-1',
               'run-' || n, 'rename_project', 'configuration_write',
               'succeeded', %s
        FROM generate_series(1, 250) AS series(n)
        """,
        (user_name, project_id, old),
