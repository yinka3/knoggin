"""Tests for jobs/dlq.py — DLQ replay with stage-aware retry.

All dependencies mocked. No Redis, Memgraph, or LLM calls.
"""
...
from shared.models.schema.dtypes import BatchResult, EntityPair, MessageConnections
...
def make_ctx(dlq_items=None):
    """Build JobContext with mocked Redis. dlq_items is a list of dicts to populate the queue."""
    redis = MagicMock()
...
class TestDLQReplayJob:
    ...
    def test_stale_entities_filtered(self):
        job, resolver, *_ = make_dlq_job()
        # Only entity 1 still exists
    ...
    async def test_success(self):
        ...
        with patch("jobs.dlq.emit", new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_no_valid_entities_after_validation(self):
        job, resolver, _, write_to_graph = make_dlq_job()
        resolver.entity_profiles = {}  # all stale
        ...
        assert result is True  # handled, not retried
    ...
    @pytest.mark.asyncio
    async def test_full_reprocess_success(self):
    ...
    @pytest.mark.asyncio
    async def test_no_messages_skips(self):
    ...
    @pytest.mark.asyncio
    async def test_reprocess_fails(self):
        job, _, processor, _ = make_dlq_job()
    ...
    @pytest.mark.asyncio
    async def test_retry_fails_requeued(self):
        job, _, processor, _ = make_dlq_job()
        ...
        # Should re-queue with incremented attempt
        rpush_calls = ctx.redis.rpush.call_args_list
        # Find the re-queue call (not the park call)
    ...
    async def test_corrupt_json_parked(self):
        """Corrupt entry in Redis should be parked, not crash the job."""
        job, *_ = make_dlq_job()
    ...
    async def test_circuit_breaker(self):
        """5 consecutive failures should halt the batch."""
        job, _, processor, _ = make_dlq_job()
        ...
        assert processed_count <= 6  # 5 failures + circuit break
    ...
    def test_partial_update(self):
        ...
        assert job.interval == 120
        assert job.batch_size == 50  # unchanged