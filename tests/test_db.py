"""Tests for db/reader.py, db/writer.py, db/store.py, and db/query_tools.py.

Uses a mocked Neo4j driver to test Python-side logic including parameter
handling, result hydration, error paths, and delegation.
"""
...
class TestGraphReader:
    ...
    # --- get_parent_entities / get_child_entities ---
    ...
    # --- has_direct_edge / has_hierarchy_edge ---
    ...
    # --- get_neighbor_entities (alias for get_neighbor_ids with full hydration isn't exactly the method) ---

    def test_get_neighbor_entities_happy(self, reader):
        rdr, session = reader
        records = [
            make_record({"id": 5, "name": "Bob"})
        ]
        session.run.return_value.__iter__ = lambda self: iter(records)
...
class TestGraphWriter:
    ...
    # --- update_entity_profile / embedding / checkpoint ---
...
class TestGraphToolQueries:
    ...
    def test_find_path_filtered_with_active_topics(self, tools):
        tq, session = tools
        # `has_inactive` is True, so it falls back to _find_active_only_path
    ...
    def test_find_path_filtered_fallback_to_shortest(self, tools):
        tq, session = tools
        # `has_inactive` is True, _find_active_only_path returns None, so it returns empty with is_hidden=True
        tq._find_shortest_path = MagicMock(return_value=(["A", "B"], ["T1", "T2"], [[], []], True))
    ...
    def test_find_path_filtered_no_active_topics(self, tools):
        tq, session = tools
        # `has_inactive` is False, so it directly uses _find_shortest_path and skips _find_active_only_path
...
class TestMemGraphStore:
    ...
    def test_verify_conn_sleep_calls(self, mock_gdb):
        ...
        with patch("db.store.time.sleep") as mock_sleep:
            MemGraphStore(uri="bolt://localhost:7687")
