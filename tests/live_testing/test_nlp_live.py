"""
Tests for main/nlp_pipe.py — live LLM (VP-01) extraction quality.

This module validates the extraction quality of the NLPPipeline using live LLM calls.
It uses real spaCy, GLiNER, and LLM services. Note that these tests incur LLM costs.

Prerequisites:
- OPENROUTER_API_KEY set in the environment or .env file.
...
# --- Live Extraction Quality Tests ---

class TestLiveExtractionQuality:
    """End-to-end tests for extraction quality across various conversation domains."""
    ...
    async def test_grad_student_entities(self, spacy_model, gliner_model, llm_service):
        """Should extract: Stanford, Emily Zhang, Chen Wei, DeepMind. InstructGPT/NeurIPS borderline."""
        pipeline = make_live_pipeline(spacy_model, gliner_model, llm_service)
    ...
    async def test_casual_entities(self, spacy_model, gliner_model, llm_service):
        """Should extract: Kenji, Notion. Oppenheimer/AMC/Ichiran are borderline."""
        pipeline = make_live_pipeline(spacy_model, gliner_model, llm_service)
    ...
    async def test_assistant_turn_extraction(self, spacy_model, gliner_model, llm_service):
        """Entities in assistant messages should also be extracted.
        Should find: Lucia, Figma, Vercel, Jake."""
        pipeline = make_live_pipeline(spacy_model, gliner_model, llm_service)
    ...
    async def test_hobby_entities(self, spacy_model, gliner_model, llm_service):
        """Should extract: Dmitri, Chopin. Moscow Conservatory and Zimerman are borderline."""
        pipeline = make_live_pipeline(spacy_model, gliner_model, llm_service)
    ...
    async def test_known_entity_priority(self, spacy_model, gliner_model, llm_service):
        """Pre-registered entities should still appear with correct topic/type from profile."""
    ...
    async def test_topic_assignment_accuracy(self, spacy_model, gliner_model, llm_service):
        """Entities should be assigned to correct topics based on label schema."""
        pipeline = make_live_pipeline(spacy_model, gliner_model, llm_service)
    ...
    async def test_ubiquity_filter(self, spacy_model, gliner_model, llm_service):
        """Mass-market brands as background context should be filtered.
        Starbucks with employment relationship should be extracted.
        Daniela and Plaid should be extracted as real entities."""
        pipeline = make_live_pipeline(spacy_model, gliner_model, llm_service)
        ...
        # People and companies with real relationships should survive
        ...
        # Zoom/iPhone as tools should ideally be filtered
        # Starbucks with "applied to work at" should survive
        # This is LLM quality dependent — we check the aspiration, not hard-fail
        if "starbucks" in names_lower:
            pass  # Good — caught the employment relationship
