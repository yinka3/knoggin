...
Covers: _normalize_timestamp, _format_timestamp, format_retrieved_messages,
        format_entity_results, format_graph_results, format_path_results,
        format_hierarchy_results, format_memory_context, format_hot_topic_context,
        format_fact_results, format_files_context.
"""
...
class TestNormalizeTimestamp:
    ...
    def test_milliseconds(self):
        ts = 1700000000_000  # same instant, ms
        result = _normalize_timestamp(ts)
    ...
    def test_out_of_bounds_high(self):
        # TS_MAX is ~4.1e9. Need a value where even ns division exceeds it.
        # 10**19 / 1e9 = 10**10 > TS_MAX
...
class TestFormatGraphResults:
    ...
    def test_unknown_shape_skipped(self):
        """Results with neither source/target nor entity are silently skipped."""
...
class TestFormatFactResults:
    ...
    def test_fallback_resolution(self):
        """Fallback branch delegates to format_retrieved_messages."""
        results = [{
            "resolution": "fallback",
            "results": [{
                "score": 0.7,
