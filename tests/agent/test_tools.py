...
All async with mocked store/redis/resolver/file_rag/memory.
...
def mock_resolver():
    resolver = MagicMock()
    resolver.resolve_entity_name.return_value = None
...
class TestGetConnections:
    ...
    async def test_hidden_connections_through_inactive_topics(self, tools, mock_resolver, mock_store):
        ...
        # First call (active topics) returns empty
        # Second call (no topic filter) returns results
...
class TestSearchFiles:
    ...
    async def test_successful_search(self, tools):
        ...
        mock_rag.search.return_value = [{"content": "found it", "score": 0.9}]
        tools.file_rag = mock_rag
...
class TestWebSearch:
    ...
    async def test_web_search_no_provider(self, tools):
        """With no search API configured, should fall back to DuckDuckGo or return error."""
        # The actual behavior depends on provider chain — just verify it doesn't crash
        with patch("agent.tools.httpx.AsyncClient") as mock_client:
...
class TestResolveEntityName:
    ...
    def test_delegates_to_resolver(self, tools, mock_resolver):
        mock_resolver.resolve_entity_name.return_value = "Alice Smith"
        result = tools._resolve_entity_name("alice")
        mock_resolver.resolve_entity_name.assert_called_once_with("alice")
        assert result == "Alice Smith"

    def test_returns_none_when_not_found(self, tools, mock_resolver):
        mock_resolver.resolve_entity_name.return_value = None
        assert tools._resolve_entity_name("nobody") is None